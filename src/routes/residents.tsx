...
import { useStore, upsertResident, deleteResident, useCurrentUser, type Resident, type Device } from "@/lib/mock-store";
...
function ResidentsPage() {
  ...
  const blank = (): Resident => ({
    ...
    state: "IDLE", mv: 0, wander: 0, presence: "ABSENT", lastActivityAt: 0, confidence: 0, online: true,
  });
  ...
  return (
    <div>
      <Header title={title} criticalCount={0} onlineCount={residents.filter((r) => r.online).length} />
      <div className="p-6 space-y-4 max-w-6xl">
        <div className="flex justify-between items-center">
          <div>
            <h1 className="text-2xl font-semibold tracking-tight mb-1">{isFacility ? "Residents & Devices" : "Home Users & Devices"}</h1>
          ...
          <button
            ...
            title={scopedDevices.length === 0 ? "먼저 장치를 등록하세요" : ""}
          >
        ...
        <div className="bg-surface border border-border rounded-lg overflow-hidden">
          <table className="w-full text-left text-sm">
            ...
            <tbody className="divide-y divide-border">
              {residents.map((r) => {
                const ids = r.deviceIds && r.deviceIds.length > 0 ? r.deviceIds : [r.deviceId].filter(Boolean);
                    ...
                    <td className="p-3 font-mono text-xs">
                      {names.length === 0 ? (
                        ...
                        <div className="flex flex-wrap gap-1">
                          {names.map((n, i) => (
                            <span key={i} className="px-1.5 py-0.5 rounded border border-border bg-background text-[10px]">{n}</span>
                          ))}
                        </div>
                      )}
                    </td>
                    <td className="p-3 font-mono text-xs">
                      {r.thresholdOverride != null ? <span className="text-warning">{r.thresholdOverride}</span> : <span className="text-muted">global</span>}
                    </td>
                    <td className="p-3">
                      <span className={`text-[10px] font-mono uppercase ${r.online ? "text-success" : "text-muted"}`}>
                        {r.online ? "● online" : "○ offline"}
                      </span>
                    </td>
                    <td className="p-3 text-right space-x-2">
                      <button onClick={() => setEditing(r)} className="text-[10px] font-mono uppercase text-muted hover:text-foreground">edit</button>
                      <button onClick={() => { deleteResident(r.id); toast(`${r.name} 삭제됨`); }} className="text-[10px] font-mono uppercase text-primary hover:brightness-110">delete</button>
                    </td>
                  </tr>
                );
              })}
              {residents.length === 0 && (
                <tr><td colSpan={8} className="p-8 text-center text-muted text-xs">
                  {scopedDevices.length === 0
                    ? "등록된 장치가 없습니다. 장치 설정에서 먼저 ESP32를 등록하세요."
        ...
        {editing && (
          <EditModal
            ...
            onSave={(r) => {
              // 다중 매핑에서 주 장치 보정
              const ids = r.deviceIds && r.deviceIds.length > 0 ? r.deviceIds : (r.deviceId ? [r.deviceId] : []);
...
function EditModal({
  resident, devices, isFacility, onClose, onSave,
}: {
  resident: Resident; devices: Device[]; isFacility: boolean;
  onClose: () => void; onSave: (r: Resident) => void;
}) {
  const [r, setR] = useState<Resident>({
    ...resident,
    deviceIds: resident.deviceIds && resident.deviceIds.length > 0
      ? resident.deviceIds
      : (resident.deviceId ? [resident.deviceId] : []),
  });
    ...
    <div className="fixed inset-0 bg-black/70 z-40 flex items-center justify-center p-6">
      <div className="bg-surface border border-border rounded-lg max-w-lg w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-6 grid grid-cols-2 gap-4">
          <Field label="이름"><input value={r.name} onChange={(e) => setR({ ...r, name: e.target.value })} className={inputCls} /></Field>
          <Field label={isFacility ? "방 번호" : "주 사용 공간"}><input value={r.room} onChange={(e) => setR({ ...r, room: e.target.value })} className={inputCls} /></Field>
          <Field label="나이"><input type="number" value={r.age} onChange={(e) => setR({ ...r, age: Number(e.target.value) })} className={inputCls} /></Field>
          <Field label={isFacility ? "담당 요양사" : "관계 / 메모"}><input value={r.caregiver} onChange={(e) => setR({ ...r, caregiver: e.target.value })} className={inputCls} /></Field>
          <Field label="MV Threshold Override">
            <input type="number" step="0.1" placeholder="global 사용" value={r.thresholdOverride ?? ""} onChange={(e) => setR({ ...r, thresholdOverride: e.target.value === "" ? undefined : Number(e.target.value) })} className={inputCls} />
          </Field>
          <Field label="온라인">
            <label className="flex items-center gap-2 text-sm">
              <input type="checkbox" checked={r.online} onChange={(e) => setR({ ...r, online: e.target.checked })} />
              online
            </label>
          </Field>

          <div className="col-span-2">
            <div className="text-[10px] font-mono text-muted uppercase mb-1">매핑 장치 · Devices ({(r.deviceIds ?? []).length})</div>
            <div className="text-[10px] text-muted mb-2">한 명당 여러 장치를 매핑할 수 있습니다. ★ 표시가 주(primary) 장치입니다.</div>
            <div className="border border-border rounded divide-y divide-border max-h-56 overflow-y-auto">
              {devices.length === 0 && (
                <div className="p-3 text-xs text-muted text-center">사용 가능한 장치가 없습니다.</div>
              )}
              {devices.map((d) => {
                const checked = (r.deviceIds ?? []).includes(d.id);
                const isPrimary = r.deviceId === d.id;
                return (
                  <label key={d.id} className={`flex items-center gap-2 px-3 py-2 text-sm cursor-pointer ${checked ? "bg-primary/5" : ""}`}>
                    <input type="checkbox" checked={checked} onChange={() => toggleDevice(d.id)} />
                    <div className="flex-1">
                      <div className="font-medium">{d.name} <span className="text-[10px] font-mono text-muted">{d.room}</span></div>
        ...
        <div className="p-4 border-t border-border flex justify-end gap-2">
          ...
          <button
            onClick={() => onSave(r)}
            disabled={!r.name || (r.deviceIds ?? []).length === 0}
            className="px-4 py-2 bg-primary text-primary-foreground rounded text-xs font-mono uppercase font-bold disabled:opacity-40"
          >Save</button>
        </div>
      </div>
...
const inputCls = "w-full bg-background border border-border rounded px-2 py-1.5 text-sm font-mono focus:outline-none focus:border-muted";
