          ...
          <>
            ...
            <section>
              <div className="flex justify-between items-center mb-3">
                ...
                <button
                  onClick={() => setEditing(blank(undefined))}
  ...
  return (
    <div className="bg-surface border border-border rounded p-4">
      <div className="flex justify-between items-start mb-2">
        ...
        <span
          className={`text-[10px] font-mono uppercase px-2 py-0.5 rounded border ${enabled ? "text-success border-success/30 bg-success/10" : "text-muted border-border"}`}
        >
          {enabled ? "ACTIVE" : "OFF"}
        </span>
      </div>
      ...
      <div className="bg-surface border border-border rounded-lg max-w-md w-full">
        ...
        <div className="p-6 space-y-4">
          ...
          <F label="역할">
            <select
              value={r.role}
              onChange={(e) => setR({ ...r, role: e.target.value as any })}
              className={cls}
            >
              <option>가족</option>
              <option>요양사</option>
...
function RealNtfySection() {
  ...
  const handleAdd = async () => {
    if (!newTopic) return;
    try {
      await addNtfyRecipient({
        name: newName || undefined,
        topic: newTopic,
        server: newServer,
        notify_fall_enabled: newNotifyFall,
      });
            ...
            {recipients.map((r) => (
              <tr key={r.id} className="hover:bg-black/5">
                ...
                <td className="p-3">
                  <label className="flex items-center gap-1.5 cursor-pointer">
                    <input
                      ...
                      onChange={(e) => {
                        updateNtfyRecipient(r.id, { notify_fall_enabled: e.target.checked }).catch(
                ...
                <td className="p-3 font-mono text-[10px]">
                  ...
                  <div className={r.failed_count > 0 ? "text-warning" : "text-muted"}>
                    Fail: {r.failed_count}
                  </div>
                  <div className={r.dropped_count > 0 ? "text-primary" : "text-muted"}>
                    Drop: {r.dropped_count}
                  </div>
                </td>
                <td className="p-3">
                  {r.last_error ? (
                    <div className="text-[9px] font-mono text-primary truncate max-w-[150px]">
                      {r.last_error}
                    </div>
                  ) : (
                ...
                <td className="p-3 text-right space-x-2">
                  ...
                  <button
                    onClick={() => removeNtfyRecipient(r.id).then(() => toast("삭제됨"))}
                    className="text-[10px] font-mono uppercase text-primary"
                  >
                    Delete
                  </button>
          ...
          <div className="bg-surface border border-border rounded-lg max-w-sm w-full p-6 space-y-4">
            ...
            <F label="설정">
              <label className="flex items-center gap-2 text-sm font-mono text-muted">
                <input
                  type="checkbox"
                  checked={newNotifyFall}
                  onChange={(e) => setNewNotifyFall(e.target.checked)}
                  className="accent-primary"
                />
                낙상 알림 필수 수신
              </label>
